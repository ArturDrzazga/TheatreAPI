...
class Ticket(models.Model):
    ...
    def validate_ticket(seat: int, row:int, theatre_hall: TheatreHall):
        if not (1 <= seat <= theatre_hall.seats_in_row):
            raise ValidationError(f"Seat must be in range between 1 "
                                  f"and {theatre_hall.seats_in_row}")
        if not (1 <= row <= theatre_hall.rows):
            raise ValidationError(f"Row must be in range between 1 "
                                  f"and {theatre_hall.rows}")
    ...
    def __str__(self) -> str:
        return (f"{self.performance.play.title} in "
                ...
                f"{self.performance.show_time} row: {self.row} "
                f"seat: {self.seat}")
